    ...
    for (auto &fptr : functions)
    {
        ...
        for (auto &bbptr : f.blocks)
        {
            ...
            for (auto &instPtr : bbptr->instrs)
            {
                ...
                {
                    std::string op;
                    if (inst.op == Instruction::Op::Add)
                        op = "add";
                    if (inst.op == Instruction::Op::Sub)
                        op = "sub";
                    if (inst.op == Instruction::Op::Mul)
                        op = "mul";
                    if (inst.op == Instruction::Op::Div)
                        op = "div";
                ...
                case Instruction::Op::Ret:
                    if (!inst.operands.empty())
                        out << "ret " << valToStr(inst.operands[0]) << "\n";
                    else
                        out << "ret\n";
                    break;
                ...
                //     break;
                case Instruction::Op::CmpEq:
                case Instruction::Op::CmpNe:
                case Instruction::Op::CmpLt:
                case Instruction::Op::CmpLe:
                case Instruction::Op::CmpGt:
                case Instruction::Op::CmpGe:
                {
                    std::string cmpOp;
                    if (inst.op == Instruction::Op::CmpEq)
                        cmpOp = "cmpeq";
                    if (inst.op == Instruction::Op::CmpNe)
                        cmpOp = "cmpne";
                    if (inst.op == Instruction::Op::CmpLt)
                        cmpOp = "cmplt";
                    if (inst.op == Instruction::Op::CmpLe)
                        cmpOp = "cmple";
                    if (inst.op == Instruction::Op::CmpGt)
                        cmpOp = "cmpgt";
                    if (inst.op == Instruction::Op::CmpGe)
                        cmpOp = "cmpge";
                    out << cmpOp << " "
                        << valToStr(inst.operands[0]) << ", "
                        << valToStr(inst.operands[1]) << "\n";
                    break;
                }
